...
`/auth/login` generates random state and a PKCE verifier. It stores them for five minutes in an AES-256-GCM encrypted, authenticated, HttpOnly, SameSite=Lax cookie and redirects to `/mock-provider/authorize` with the S256 challenge. The mock provider issues a short-lived code. `/auth/callback` decrypts the attempt cookie, checks expiry and constant-time state equality, and exchanges the code with `/mock-provider/token` using the verifier. After identity mapping, it creates a hashed `AuthSession` row in PostgreSQL and issues a one-hour encrypted cookie containing the raw session token. `/dashboard` validates both the cookie and active database row. Current-session logout revokes one row; all-device logout revokes every active row for the user.
